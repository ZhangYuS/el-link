...
class Dygraph_linear_warmup(D.PolynomialDecay):
    def __init__(self, learning_rate, decay_steps, warmup_steps, end_learning_rate, *args, **kwargs):
        assert warmup_steps < decay_steps
        super(Dygraph_linear_warmup, self).__init__(learning_rate, decay_steps, end_learning_rate, *args, **kwargs)
        self.wp = warmup_steps
        self.lr = learning_rate

    def step(self):
        tmp_step_num = self.step_num
        if tmp_step_num < self.wp:
            lr_wp = self.create_lr_var(self.lr * (self.step_num/self.wp))
        else:
            lr_wp = super().step()
        return lr_wp

class Dygraph_exp_warmup(D.NaturalExpDecay):
    def __init__(self, learning_rate, decay_steps, decay_rate, warmup_steps):
        assert warmup_steps < decay_steps
        super(Dygraph_exp_warmup, self).__init__(learning_rate, decay_steps, decay_rate)
        self.wp = warmup_steps
        self.lr = learning_rate
...
if __name__ == '__main__':
    ...
    with D.guard(F.CPUPlace()):
        d = Dygraph_linear_warmup(learning_rate=1e-3, decay_steps=10000, end_learning_rate=1e-7, warmup_steps=10)
        while True:
            print(d(), d.step(), d.step_num)
