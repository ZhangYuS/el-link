...
class PretrainedModel(object):
    ...
    def from_pretrained(cls, pretrain_dir_or_url, force_download=False, **kwargs):
        ...
        cfg_dict = json.loads(open(config_path).read())
        model = cls(cfg_dict, **kwargs)
...
class ErnieModel(D.Layer, PretrainedModel):
    def __init__(self, cfg, num_out_pooler=1, name=None):
        """
        ...
        self.num_out_pooler = num_out_pooler
        ...
        if self.num_out_pooler == 1:
            self.pooler = _build_linear(cfg['hidden_size'], cfg['hidden_size'], append_name(name, 'pooled_fc'),
                                        initializer, act='tanh')
            self.poolers = None
        elif num_out_pooler > 1:
            self.poolers = D.LayerList([_build_linear(cfg['hidden_size'], cfg['hidden_size'], append_name(name, f'pooled_fc{i}'),
                                        initializer, act='tanh') for i in range(self.num_out_pooler)])
            self.pooler = None
        else:
            raise ValueError
    ...
    def forward(self, src_ids, sent_ids=None, pos_ids=None, input_mask=None, attn_bias=None, past_cache=None,
                use_causal_mask=False, feat_embed=None, src_embed=None, **kwargs):
        """
        Args:
        ...
        if self.pooler is not None and self.num_out_pooler == 1:
            pooled = self.pooler(encoded[:, 0, :])
        elif self.num_out_pooler > 1:
            # 每个poolers使用一个单独的layer
            assert self.num_out_pooler < 6
            pooled = []
            for i in range(self.num_out_pooler):
                pooled.append(self.poolers[i](hidden_list[-(i + 1)][:, 0, :]))
        else:
            pooled = None
